			...
			//##################################################################
				...
				if ($activated == "yes") {
					...
					if ($data['rel_time'] == 'normal') {
						...
						if ($data['rel_state'] == "0") {
							$stream = ssh2_exec($connection, '/var/www/yana-server/plugins/relay/radioEmission '. $data['rel_pin'] .' '. $data['rel_tel'] .' '. $data['rel_rec'] .' on');
							...
							$stream = ssh2_exec($connection, '/var/www/yana-server/plugins/relay/radioEmission '. $data['rel_pin'] .' '. $data['rel_tel'] .' '. $data['rel_rec'] .' off');
